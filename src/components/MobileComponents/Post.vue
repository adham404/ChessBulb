  ...
  <div>
    <v-sheet class="mt-8">
      ...
      <v-row class="mt-1 mb-1" justify="space-between">
        <span cols="4" class="ml-4 text-subtitle-2"
          >{{ Match.noOfBrilliants }} brilliants</span
        >
        <span cols="4" class="mr-4 text-subtitle-2"
          >{{ Match.noOfAnalysis }} Analyses</span
        >
      </v-row>
      <v-divider> </v-divider>
...
export default {
  ...
  methods: {
    ...
    BrilliantClicked() {
      if (
        this.Match.BrilliantUsers.includes(this.UserId) &&
        !this.Match.noOfBrilliants == 0
      ) {
    ...
    copy() {
      this.$copyText(this.link + this.Match.MatchId).then(
        function(e) {
          alert("Copied");
          console.log(e);
        },
