<template>
    <div class = "white">
        ...
        <v-container rounded class="pt-0">
            <h1 class=" font-weight-normal text-h2 mt- pl-2 mb-2 pb-2 pt-0" style="color: #215679;">Instructor Login</h1>
            ...
            <v-row justify="center">
              <v-btn class="mb-5 primary text-h6" cols= "12" x-large width="90%" @click="Login">Login</v-btn>
            </v-row>
...
export default {
    ...
    computed:{
        ...mapGetters(['GetIAmInstructor'])
    },
//DONE define Login Function (1min)
    methods:{
        ...mapActions(['CheckInstructorValidaty']),
        async Login()
        {
        ...
        {            
            //DONE use firebase Auth to login using the two data properties (UserEmail) and (UserPassword) (25min)
                ...
                await auth.onAuthStateChanged( async (user)=>{
                  if (user) {
                    self.ErrorMessage = "";
                    //Check Whether the user is an instructor or not
                    await this.CheckInstructorValidaty(user.uid);
                    if(this.GetIAmInstructor)
                    {
                        console.log("I am : "+ this.GetIAmInstructor);
                        this.$router.push('/');

                    }
                    else
                    {
                      self.ErrorMessage = "Not Registered as an Instructor";
                    }
