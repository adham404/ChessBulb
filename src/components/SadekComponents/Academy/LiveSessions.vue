<template>
  <div>
    //TODO using the (SearchEngine Component)
    //TODO using the (Filter Component)
    //TODO loop through liveSessions
    //TODO using the (LiveSessionCard Component) pass the data of each live session, the userId and AcademyID (2 minutes)
  </div>
</template>

<script>
//TODO importing the (LiveSessionCard Component)
//TODO importing firebase
export default {
  //TODO using props to get the Academy data
  //TODO get userID
  //TODO make an array for liveSessions
  //TODO in "mounted" get the live session of the academy using its ID and save them in the array
};
</script>

<style scoped>
</style>