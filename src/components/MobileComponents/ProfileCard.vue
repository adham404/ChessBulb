<template>
    <div class="my-8">
      ...
      <v-row class="mt-1 px-1 pb-1">
        ...
        <v-col cols="9">
          <span class="text-h6">{{FollowerData.FirstName}} {{FollowerData.LastName}}</span>
          <br>
          <span class="text-caption">{{FollowerData.UserBio}}</span>
          <br>
          <v-row style="justify-content:space-between;" class="mt-1 px-1 pb-1">
          <v-btn xs v-if="!FollowingState" @click="Follow" class="text-capitalize primary"  height="25">follow</v-btn>
          <v-btn xs v-if="FollowingState" @click="UnFollow" class="text-capitalize primary"  height="25">Unfollow</v-btn>
          <v-btn style="margin-right:20%;" xs @click="VisitProfile" class="text-capitalize primary"  height="25">Visit Profile</v-btn>            
          </v-row>
        </v-col>
      </v-row>
      </v-sheet>
