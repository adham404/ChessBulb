<template>
  <v-sheet style="margin-top:10%">
      <v-row>
          ...
          max-height="150"
          max-width="50%"
          :src="CurrentImgUrl ? CurrentImgUrl : '/img/icons/pexels-pixabay-220453.jpg'"          >        
          </v-img>
          <v-col cols="2">
...
methods:{
    ...mapActions(['UploadProfilePic','fetchProfilePic','RemoveProfilePic','SaveProfileSettingDataEdit','fetchUserInfo']),
    Edit()
    {
        this.EditFlag = true
    },
    ...
    async SaveNewPass()
    {
        if((this.NewPass == this.ConfirmPass) && this.ConfirmPass && this.NewPass)
        {
            
            await firebase.auth().currentUser.updatePassword(this.NewPass).then(() => {
    ...
    async Remove()
    {
        ...
        this.CurrentImgUrl = "";
            ...
            async UploadPhoto()
            {
                ...
                PhotoData.Url = "UserProfilePics/" + this.GETUserFULLDATA.UserId + "/" + this.PicData[0].name + ".jpg";
