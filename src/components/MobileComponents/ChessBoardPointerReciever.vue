...
methods:{
    DrawPointer(fromx, fromy, tox, toy, StreamerBoardWidth)
    {
        var canv = document.getElementById("Canva1");
        var context = canv.getContext("2d");
        var ratio = this.ActiveWidth/StreamerBoardWidth;
...
async mounted()
{
    ...
    await EventBus.$on("SendArrow",(PointerData) => { 
        console.log("Data Recieved baby");
        this.DrawPointer(PointerData.x1,PointerData.y1,PointerData.x2,PointerData.y2,PointerData.StreamerBoardWidth);
    })
    EventBus.$on("displayboardfen", move => {
      this.Board.position(move);
    });
    EventBus.$on("ClearArrows", () => {
        this.Erase()
    })
