...
const actions = {
    async EditMyAcademyBio({state},data)
    {
        if(data != "")
        {
            ...
            await db.collection("Academies").doc(state.MyAcademyData.AcademyId).update({
                About: data
            })
        }
    },
    async FetchLiveCardDataForDashboard({state})
    {
        state.LiveSessions = []
        var db = firebase.firestore()
        await db.collection("Lives").get().then((snap) => {
            snap.forEach((doc) => {
    ...
    async FetchMyAcademyData({state})
    {
        ...
        var db = firebase.firestore()
        await db.collection("Academies").get().then((snap) => {
                snap.forEach((doc) => {
