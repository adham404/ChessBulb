    ...
    <div class="my-8">
      ...
      <v-row class="mt-1 px-1 pb-1">
        ...
        <v-col cols="9">
          ...
          <br>
          <span class="text-caption">by {{FullName}} (instructor at Smouha Chess Academy)</span>
          <br>
          <v-rating
            hover
            length="5"
          ...
          <v-btn v-if="preview" xs class="text-capitalize primary" height="25" @click="() => {$router.push({path:`/CoursePage/${GetListOfCourses[order].CourseId}`, query:{CourseID: GetListOfCourses[order].CourseId}})}">Open</v-btn>
        </v-col>
      </v-row>
    </v-sheet>
...
import firebase from "firebase"
    ...
    export default {
        ...
        async mounted()
      {
        ...
        console.log("Instructoo id: "+ this.GetListOfCourses[this.order].InstructorId);
        var db = firebase.firestore()
        await db.collection("Users").doc(this.GetListOfCourses[this.order].InstructorId).get().then((doc) => {
